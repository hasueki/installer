...
func validateDedicatedHosts(dhosts []ibmcloud.DedicatedHost, itype string, zones []string, path *field.Path) field.ErrorList {
	...
	// Check if length of dedicated hosts matches zones
	...
	for i, dhost := range dhosts {
		// If 'id' is absent, 'profile' and 'zone' is required
		if dhost.ID == "" {
			if dhost.Profile == "" {
				allErrs = append(allErrs, field.Invalid(path.Index(i).Child("profile"), dhost.Profile, "profile must be set"))
			}
			if dhost.Zone == "" {
		...
		// Dedicated host zone must be in platform zones
		if dhost.Zone != "" && zones != nil {
			isPresent := false
			...
			if !isPresent {
				allErrs = append(allErrs, field.Invalid(path.Index(i).Child("zone"), dhost.Zone, fmt.Sprintf("zone not in expected list of zones (%s)", zones)))
			}
		}
	}
