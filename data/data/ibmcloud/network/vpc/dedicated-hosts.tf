locals {
  ...
  dhosts_master_merged = [
    for dhost in var.dedicated_hosts_master :
    lookup(dhost, "id", "") == ""
    ? ibm_is_dedicated_host.control_plane[index(ibm_is_dedicated_host.control_plane.*.zone, dhost.zone)].id
    : dhost.id
  ]
}
...
resource "ibm_is_dedicated_host_group" "control_plane" {
  ...
  name           = "${local.prefix}-dgroup-control-plane-${local.dhosts_master_create[count.index].zone}"
  ...
  zone           = local.dhosts_master_create[count.index].zone
}

resource "ibm_is_dedicated_host" "control_plane" {
  count = length(local.dhosts_master_create)

  name           = "${local.prefix}-dhost-control-plane-${local.dhosts_master_create[count.index].zone}"
...
resource "ibm_is_dedicated_host_group" "compute" {
  ...
  name           = "${local.prefix}-dgroup-compute-${local.dhosts_worker_create[count.index].zone}"
  ...
  resource_group = var.resource_group_id
  zone           = local.dhosts_worker_create[count.index].zone
}

resource "ibm_is_dedicated_host" "compute" {
  count = length(local.dhosts_worker_create)

  name           = "${local.prefix}-dhost-compute-${local.dhosts_worker_create[count.index].zone}"
